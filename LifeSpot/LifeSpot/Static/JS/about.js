...
slides.forEach(slide => {
  ;['dragstart', 'dragleave'].forEach(ev =>
    slide.addEventListener(ev, getMouseMoves)
  )
  slide.addEventListener('dragend', plusSlides)
})
...
function getMouseMoves(e) {
  if (e.buttons === 1) mouseMove.startX = e.clientX
  if (e.buttons === 0) mouseMove.endX = e.clientX
}

function getMouseDirection() {
  return mouseMove.endX - mouseMove.startX
}
...
function plusSlides() {
  if (this.classList.contains('prev') || getMouseDirection() < 0)
    showSlides(--slideIndex)
  if (this.classList.contains('next') || getMouseDirection() > 0)
    showSlides(++slideIndex)
}
